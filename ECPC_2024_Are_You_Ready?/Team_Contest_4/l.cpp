// Problem: L. Median of the Array
// Contest: Codeforces - Team Contest #4
// URL: https://codeforces.com/group/qxlLGDBwj5/contest/533062/problem/L
...
void ITcharDo()
{
     ll n,count=0; cin>>n; vll; cins(v,n);
     map<ll,ll>mp;
     map<ll,pair<ll,ll>>mp2;
     FOR(i,0,n)mp[v[i]]++;
     for(auto& [f, s] : mp)
     {
     	mp2[f]={count,(n-(count+s))};
     	count+=s;
     }
     for(auto& [f, s] : mp)
     {
     	if(s>1)
     	{
     		...
     		if(ff==ss){cout<<YS; return ;}
     		if(ff==0&&ss<=s){cout<<YS; return ;}
     		if(ss==0&&((s-ff)>=2)){cout<<YS; return ;}
     		
     		ll s2=s-2;
     		if(ff>ss)
     		{
     			if((ff-ss)<=s2){cout<<YS; return ;}
     		}
     		if(ss>ff)
     		{
     			if(((ss-ff)-2)<=s2){cout<<YS; return ;}
     		}
